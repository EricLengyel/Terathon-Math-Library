...
Line3D Terathon::Transform(const Line3D& l, const Motor4D& Q)
{
	...
	return (Line3D(vx + ((ryrw * vz - rzrw * vy) + vy * rxry + vz * rzrx - vx * (ry2 + rz2)) * 2.0F,
...
Plane3D Terathon::Transform(const Plane3D& g, const Motor4D& Q)
{
	...
	float gr = g.x * rx + g.y * ry + g.z * rz;
	float gu = g.x * ux + g.y * uy + g.z * uz;

	return (Plane3D(g.x + ((ry * g.z - rz * g.y) * rw + g.y * rxry + g.z * rzrx - g.x * (ry2 + rz2)) * 2.0F,
	                g.y + ((rz * g.x - rx * g.z) * rw + g.x * rxry + g.z * ryrz - g.y * (rz2 + rx2)) * 2.0F,
	                g.z + ((rx * g.y - ry * g.x) * rw + g.x * rzrx + g.y * ryrz - g.z * (rx2 + ry2)) * 2.0F,
	                g.w + (gr * uw - gu * rw + g.x * (ry * uz - rz * uy) + g.y * (rz * ux - rx * uz) + g.z * (rx * uy - ry * ux)) * 2.0F));
}
