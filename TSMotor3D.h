...
namespace Terathon
{
	...
	///
	/// The \c Motor3D class encapsulates a 3D motion operator (motor), also known as a dual quaternion.
	...
	class Motor3D
	{
		public:
			...
			///
			/// The \c Unitize() function multiplies a motor by the inverse magnitude of its weight, which is the quaternion
			...
			/// given by the \c angle parameter about the axis through the origin given by the \c axis parameter.
			...
			///
			/// The \c MakeScrew() function returns a motor representing a rotation through the angle
			/// given by the \c angle parameter about the line given by the \c axis parameter and a
			/// translation along that line of the distance given by the \c disp parameter.
			...
			///
			/// The \c GetPosition() function calculates the 3D point that results from transforming the origin
			...
			///
			/// The \c GetTransformMatrix() function converts a motor to the Transform3D object that
			...
			///
			/// The \c GetInverseTransformMatrix() function converts a motor to the inverse of the Transform3D object that
			...
			///
			/// The \c GetTransformMatrices() function converts a motor to the Transform3D object that represents the same
			...
			///
			/// This function expects the matrix \c M to have a determinant of +1, and it expects the upper-left 3&#x202F;&times;&#x202F;3
	...
	/// @brief Returns the negation of the 3D motor \c Q.
	/// @related Motor3D
	...
	/// @brief Returns the product of the 3D motor \c Q and the scalar \c n.
	/// @related Motor3D
	...
	/// @brief Returns the product of the 3D motor \c Q and the inverse of the scalar \c n.
	/// @related Motor3D
	...
	/// @brief Returns a boolean value indicating whether the two 3D motors \c a and \c b are equal.
	/// @related Motor3D
	...
	/// @brief Returns a boolean value indicating whether the two 3D motors \c a and \c b are not equal.
	/// @related Motor3D
	...
	/// @brief Returns the geometric antiproduct of the 3D motor \c Q and the quaternion \c r.
	/// @relatedalso Motor3D

	TERATHON_API Motor3D operator *(const Motor3D& Q, const Quaternion& r);

	/// @brief Returns the geometric antiproduct of the quaternion \c r and the 3D motor \c Q.
	/// @relatedalso Motor3D
	...
	/// @brief Transforms the 3D vector \c v with the motor \c Q.
	/// @relatedalso Motor3D
	...
	/// @brief Transforms the 3D bivector \c v with the motor \c Q.
	/// @relatedalso Motor3D
	...
	/// @brief Transforms the 3D flat point \c p with the motor \c Q.
	/// @relatedalso Motor3D

	TERATHON_API FlatPoint3D Transform(const FlatPoint3D& p, const Motor3D& Q);

	/// @brief Transforms the 3D Euclidean point \c p with the motor \c Q.
	/// @relatedalso Motor3D

	TERATHON_API Point3D Transform(const Point3D& p, const Motor3D& Q);

	/// @brief Transforms the 3D line \c l with the motor \c Q.
	/// @relatedalso Motor3D

	TERATHON_API Line3D Transform(const Line3D& l, const Motor3D& Q);

	/// @brief Transforms the 3D plane \c g with the motor \c Q.
	/// @relatedalso Motor3D
	...
	/// @brief Returns the reverse of the 3D motor \c Q.
	/// @relatedalso Motor3D
	...
	/// @brief Returns the antireverse of the 3D motor \c Q.
	/// @relatedalso Motor3D
	...
	/// @brief Returns the squared bulk norm of the 3D motor \c Q.
	/// @relatedalso Motor3D
	...
	/// @brief Returns the squared weight norm of the 3D motor \c Q.
	/// @relatedalso Motor3D
	...
	///
	/// The \c Unitize() function multiplies a 3D motor by the inverse magnitude of its weight, which is the quaternion
	/// given by its <b>e</b><sub>41</sub>, <b>e</b><sub>42</sub>, <b>e</b><sub>43</sub>, and antiscalar coordinates.
	/// After calling this function, the rotor component of the motor is unit-length. If these coordinates are all zero,
	/// then the result is undefined.
	///
